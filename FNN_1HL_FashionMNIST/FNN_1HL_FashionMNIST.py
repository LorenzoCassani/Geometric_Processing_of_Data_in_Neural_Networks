...
def save_data(dictionary_hidden, run, epoch):
    for i in range(len(dictionary_hidden)):
        filename = "{}/Runs/run_{}/data/manifold_{}_epoch_{}.dat".format(PATH, run, i, epoch+1)
        f2 = open(filename, "w+")
        f2.write("[\n")
...
class FCNN(nn.Module):
    ...
    def __init__(self, hidden_size, input_size=28*28*1, output_size=2):
        ...
        # Dataset
        self.dataset = datasets.Custom_FashionMNIST(num_data=self.num_data, batch_size=self.batch_size)
    ...
    def training_session(self, run, num_epochs, innerdata=False):
        ...
        for epoch in range(num_epochs):
            ...
            if innerdata:
                ...
                save_data(dictionary_hidden, run, epoch)
